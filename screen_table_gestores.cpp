...
Screen_Table_Gestores::Screen_Table_Gestores(QWidget *parent, bool show, QString empresa) :
    QMainWindow(parent),
...
void Screen_Table_Gestores::getGestoresFromServer(bool view)
{
    ...
    connect(this, &Screen_Table_Gestores::script_excecution_result,q,&QEventLoop::exit);
    QTimer::singleShot(DELAY, this, &Screen_Table_Gestores::getGestores);
    switch(q->exec())
    {
    ...
    case database_comunication::script_result::get_gestores_failed:
    ...
    delete q;
    qDebug()<<"-------------------------------JustgetGestores-------------------------------------------";
}
void Screen_Table_Gestores::getGestores(){
...
void Screen_Table_Gestores::fixModelForTable(QJsonArray jsonArray)
{
    ...
    mapa.insert("Código",codigo_gestor_gestores);
    mapa.insert("Gestor",gestor_gestores);
    mapa.insert("Permisos",permisos_gestores);

    QStringList listHeaders;
    listHeaders <<"Código" << "Gestor" << "Permisos";
...
void Screen_Table_Gestores::on_sectionClicked(int logicalIndex)
{
    ...
    QStringList listHeaders;
    listHeaders <<"Código" << "Gestor";

    QString columna = listHeaders.at(logicalIndex);
    QString ordenamiento = mapa.value(columna);

    jsonArrayAllGestores = ordenarPor(jsonArrayAllGestores, ordenamiento, "");
}
...
{
    oneGestorScreen = new Gestor(nullptr, false, empresa);
    connect(oneGestorScreen, &Gestor::update_tableGestores,this,
            &Screen_Table_Gestores::getGestoresFromServer);
    connect(oneGestorScreen, &Gestor::updateTareas,this,
            &Screen_Table_Gestores::on_updateTareas);
...
{
    oneGestorScreen = new Gestor(nullptr, true, empresa);
    connect(oneGestorScreen, &Gestor::update_tableGestores,this,
            &Screen_Table_Gestores::getGestoresFromServer);
    oneGestorScreen->show();
}
void Screen_Table_Gestores::on_updateTareas()
{
    emit updateTareas();
}
